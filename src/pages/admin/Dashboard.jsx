...
function ModalProduto({ produto, onClose, onSaved }) {
  ...
  async function uploadCloudinary(file) {
    ...
    return new Promise((resolve, reject) => {
      ...
      xhr.onerror = () => { setUploading(false); reject(new Error('Falha na conexão com Cloudinary.')) }
      xhr.send(fd)
    })
  }
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (selectedFile.current) {
        imagemUrl = await uploadCloudinary(selectedFile.current)
      }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60"
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm m-4">
        ...
        <div className="px-6 py-5 text-center">
          ...
          <p className="text-text-dark mb-1">Tem certeza que deseja excluir <strong>{nome}</strong>?</p>
...
export default function Dashboard() {
  ...
  const [modalProduto, setModalProduto]     = useState(null) // null = fechado, {} = novo, {id,...} = editar
  const [modalConfirmar, setModalConfirmar] = useState(null) // {id, nome}
  ...
  function handleSaved(produto) {
    setProdutos(prev => {
      const idx = prev.findIndex(p => p.id === produto.id)
      if (idx !== -1) {
        const next = [...prev]; next[idx] = produto; return next
      }
      return [produto, ...prev]
    })
    showToast(produto.criadoEm instanceof Date ? 'Produto adicionado com sucesso!' : 'Produto atualizado com sucesso!')
  }

  async function handleExcluir() {
    const { id, nome } = modalConfirmar
    ...
    <div className="min-h-screen bg-gray-50 font-[inherit]">
      ...
      <div className="flex">
        ...
        <aside className={`${sidebarOpen ? 'w-52' : 'w-0 overflow-hidden'} transition-all duration-300 bg-white shadow-md min-h-[calc(100vh-56px)] flex-shrink-0`}>
          <nav className="py-4">
            <a href="#" className="flex items-center gap-3 px-5 py-3 text-primary font-semibold bg-orange-50 border-r-4 border-primary">
                      ...
                      <tr key={p.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            ...
                            <button onClick={() => setModalConfirmar({ id: p.id, nome: p.nome })} title="Excluir"
